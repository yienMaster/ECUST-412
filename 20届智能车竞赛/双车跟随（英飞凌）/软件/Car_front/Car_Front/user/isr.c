...
IFX_INTERRUPT(cc60_pit_ch0_isr, CCU6_0_CH0_INT_VECTAB_NUM, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    if(start_flag && element_state!=IDLE)
    {
//        target_speed=target_speed_set;
    ...
    {
        target_speed=target_speed_L=target_speed_R=0;
        BUZZER(0);
        BLDC_L(0);
        BLDC_R(0);
    }
...
IFX_INTERRUPT(cc60_pit_ch1_isr, CCU6_0_CH1_INT_VECTAB_NUM, CCU6_0_CH1_ISR_PRIORITY)
{
    ...
    {
        uint8 count;
        if(zebra_state==ZEBRA_MAINTAIN)
            count=4;
        else if(zebra_state==ZEBRA_STOP)
            count=15;
        zebra_time_count++;
        if(zebra_time_count>count)
        {
            zebra_time_flag=0;
            zebra_time_count=0;
        }
    ...
    if(speedup_flag)
    {
        if(speedup_count<1) speedup_count++;
    }
    else speedup_count=0;
...
IFX_INTERRUPT(cc61_pit_ch1_isr, CCU6_1_CH1_INT_VECTAB_NUM, CCU6_1_CH1_ISR_PRIORITY)
{
    ...
    {
//        if(start_delay_count==170)
//            LIGHT(1);
        if(start_delay_count++==START_DELAY_TIME)
        {
            motor_l.out=motor_r.out=0;
            target_decision_flag=0;
            start_frontsight_count=0;
            LIGHT(1);
            BLDC_L(bldc_duty);
            BLDC_R(bldc_duty);
            start_flag=1;
            element_state=STRAIGHT;
            round_size_full=round_size_set;
            tft180_clear();
