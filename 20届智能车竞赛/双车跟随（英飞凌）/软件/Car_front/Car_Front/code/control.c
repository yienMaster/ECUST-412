...
//        start_row=start_row_set;
    ...
    if((speed_L+speed_R)/2-target_speed_set>=10&&element_state==STRAIGHT)
        start_row-=(uint8)dynamic_frontsight;
...
//    }
    ...
    if(out>=0)
    {
        target_speed_L=target_speed-1.25*out;
        target_speed_R=target_speed+0.75*out;
    }
    else
    {
        target_speed_L=target_speed-0.75*out;
        target_speed_R=target_speed+1.25*out;
    }
}
...
void Speed_Decision(void)
{
    switch(target_decision_flag)
    {
        ...
        case 2:
            ...
            {
                speedup_flag=1;
                if(white_col_mid_row<2)////////////////////////////////////
                {
                    if(target_speed<(target_speed_set+speed_increase))
                    {
                        if(target_speed<target_speed_set)
                ...
                else if(white_col_mid_row>=2 && (speed_L+speed_R)/2-target_speed_set>5)
                {
