...
def follow_line(pur, data, gyro , kp, kd, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 10000 + in_now * 0.1
...
def time_pit_handler_1(time):
    ...
    global stop_flag, w_z_real, theita_1_last, turn_count
...
while True:
    ...
    if cal_flag :
        ...
        line_pos = (0.55 * (left_n + right_n) + 0.45 * (left_m + right_m) + 0 * (left_f + right_f))*(1/2) 
        ...
        if mode_1 == 1:
            #直行状态
            if mode_2 == 1:
                line_pos = (0.3 * (left_n + right_n) + 0.4 * (left_m + right_m) + 0.3 * (left_f + right_f))*(1/2) 
                #line_pos = (0.45 * (left_n + right_n) + 0.35 * (left_m + right_m) + 0.2 * (left_f + right_f))*(1/2)
        elif mode_1 == 2:
