...
def find_road(data, width):
    ...
    for i in range(1, 53):
        ...
        if find_l == 0:
            ...
            # 计算差比和
            diff_ratio = (math.sqrt(l) - math.sqrt(r)) / (c + 1) * 255  # 避免除零
...
while True:
    ...
    if cal_flag :
        ...
        if mode_1 == 1:
            #直行状态
            if mode_2 == 1:
                line_pos = (0.2 * (left_n + right_n) + 0.35 * (left_m + right_m) + 0.45 * (left_f + right_f))*(1/2) 
                #line_pos = (0.4 * (left_n + right_n) + 0.6 * (left_m + right_m) + 0 * (left_f + right_f))*(1/2)
        elif mode_1 == 2:
