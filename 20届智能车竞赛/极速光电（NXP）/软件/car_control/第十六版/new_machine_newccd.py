...
def find_road(data, last_mid, num):
    """
    差比和滤波处理CCD数据，并同时寻找道路边线
    """
    max_data, min_data, ave_data = find_max_min_ave(data, 4)
    ...
    threshold = (max_data - min_data) * 100 / (max_data + min_data + 1) * k
    
    left_start = last_mid
    right_start = last_mid
    start_point = last_mid
    bre_l = 0
    bre_r = 0
    ...
    while(True):
        ...
        if left_start >= 15:
            if data[left_start] > ave_data:
                if data[left_start - 4] > ave_data:
                    start_point = round((left_start + left_start - 4) / 2)
                    break
        else:
            bre_l = 1
            
        if right_start <= 118:
            if data[right_start] > ave_data:
                if data[right_start + 4] > ave_data:
                    start_point = round((right_start + right_start + 4) / 2)
                    break
        else:
            bre_r = 1
            
        if bre_l == 1 and bre_r == 1:
            break
    ...
    for i in range(start_point, 110):

        l = data[i]
        r = data[i + step_road]
        
        diff_ratio = (l - r) / (l + r + 1) * 100
        
        if abs(diff_ratio) > threshold:
            right_edge = round((i + i + step_road) / 2)
            find_r = 1
                    
    for i in range(12, start_point):

        l = data[i - step_road]
        r = data[i]
        
        diff_ratio = (r - l) / (l + r + 1) * 100
        
        if abs(diff_ratio) > threshold:
            left_edge = round((i + i - step_road) / 2)
            find_l = 1
    ...
    return left_edge - 64 , right_edge - 64 , (left_edge +  right_edge + 2 * 64) / 2, road_width
...
def follow_line(pur, data, gyro , kp, kd, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 15000 + in_now * 0.1
...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.25 * (v_real ** 2 + v_pur ** 2)) * kp * offset + d_corrcte * kd * (in_now - in_last)
...
while True:
    ...
    if cal_flag :
        ...
        #75 60
        left_n, right_n, road_mid_n, road_width_n_now = find_road(ccd_data1 , road_mid_n_last, 1)
        #wireless.send_oscilloscope(road_width_n,mode_1,sign_err)
        #print(left_n, right_n)
        left_m, right_m, road_mid_m, road_width_m_now = find_road(ccd_data2 , road_mid_m_last, 2)
        left_f, right_f, road_mid_f, road_width_f_now = find_road(ccd_data3 , road_mid_f_last, 3)
        #wireless.send_oscilloscope(road_width_n, road_width_m, road_width_f)
        #print("find_road")
        '''
        if abs(last_left_n - left_n ) >= 15:
            left_n = last_left_n
        if abs(right_n - last_right_n ) >= 15:
            right_n = last_right_n
            
        if abs(left_m - last_left_m ) >= 15:
            left_m = last_left_m
        if abs(right_m - last_right_m ) >= 15:
            right_m = last_right_m
            
        if abs(left_f - last_left_f ) >= 15:
            left_f = last_left_f
        if abs(right_f - last_right_f ) >= 15:
            right_f = last_right_f
            '''
        if road_width_f_now < 40:
            road_width_f = road_width_f_now
        
        if road_width_m_now < 60:
            road_width_m = road_width_m_now
            
        if road_width_n_now < 80:
            road_width_n = road_width_n_now
            
        wireless.send_oscilloscope(left_n, right_n, left_m,right_m,left_f,right_f,line_pos,v_turn)
        '''
        状态机
        '''
        '''
        ...
        if mode_1 == 1:
            ...
            if mode_2 == 1:
                ...
                total_use = use_n + use_m + use_f
                
                if total_use != 0:
                    line_pos = (0.35 * (left_n + right_n) * use_n + 0.5 * (left_m + right_m) * use_m + use_f * 0.15 * (left_f + right_f))*(1/2) *(3 / total_use)
                else:
                    line_pos = 0
        elif mode_1 == 2:
            ...
            if mode_2 == 1:
                ...
                if mode_3 == 1:
                    ...
                    line_pos = (0.45 * (left_n + right_n) + 0.5 * (left_m + right_m) + 0.05 * (left_f + right_f))*(1/2)
                #elif mode_3 == 2:
                    if abs(line_pos - line_pos_last) >= 18:
                        line_pos = line_pos_last
                    else:
