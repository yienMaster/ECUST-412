...
def follow_line(pur, data, gyro , kp, kd, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 18000 + in_now * 0.1
...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + (0 * v_real ** 2 + 0.5 * v_pur ** 2)) * kp * offset + gyro * 1000 +d_corrcte * kd * (in_now - in_last)
...
while True:
    ...
    if cal_flag :
        ...
        wireless.send_oscilloscope(left_n, right_n, left_f,right_f,v_l,v_r, v_turn)
