...
def find_road(data, width):
    ...
    if find_l == 0 and find_r == 0:
        if left_edge == 64 and right_edge == 64:
            if data[70] > 15 and data[74] > 15 and data[80] > 15:
                left_edge = -53 + 64
                right_edge = 53 + 64
            else:
                left_edge =  64
    ...
    elif find_l and find_r == 0 and right_edge == 64:
        right_edge = 53 + 64
        
    elif find_r and find_l == 0 and left_edge == 64:
        left_edge = -53 + 64
...
def follow_line(pur, data, gyro , kp, kd, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 12000 + in_now * 0.1
...
straight_road_width_n = 66
straight_road_width_m = 60
straight_road_width_f = 40
...
def time_pit_handler_1(time):
    ...
    global in_dis_last, v_turn, line_pos, d_corrcte_last, road_mid_n, road_mid_m, R_turn
    ...
    if run_flag:
        if turn_count == 3:
            ...
            #print(1)
            if abs(road_mid_n - road_mid_m) >= 5 and R_turn != 0:
                
                if R_turn > 0.8:
                    R_turn = 0.8
                
                if road_mid_m * road_mid_n == 0:
                    v_turn = 0
                else:
                    v_turn = r_turn * (v_l + v_r) /(R_turn * t_turn) * (road_mid_m) / abs(road_mid_m)
                ...
                wireless.send_oscilloscope(road_mid_n, road_mid_m, v_turn, R_turn)
            else:
...
while True:
    ...
    if cal_flag :
        ...
        road_mid_n = left_n + right_n
        road_mid_m = left_m + right_m
        ...
        if m == 0:
            ...
            x_R =  ((- x1 * x1 / (2 * y1) + (x2 * x2 - x1 * x1) / (2 * (y2 - y1)) + (y1 + y2) / 2)
                            / m)
            y_R = - (x1 / y1) * x_R + x1 * x1 / (2 * y1) + y1 / 2
            R_turn = math.sqrt(x_R * x_R + y_R * y_R)
