    ...
    if run_flag:
        if turn_count == 3:
            ...
            if abs(v_turn_last - v_turn) <= 800:
                v_turn_last = v_turn
            elif abs(v_turn_last - v_turn) <= 1300:
                v_turn = 0.2 * v_turn_last + 0.8 * v_turn
                v_turn_last = v_turn
            elif abs(v_turn_last - v_turn) <= 2000:
        ...
        if ticker_count % 10 == 0:
            ...
            if abs(v_l) >= pur_v + 40 or abs(w_z_real) > 1.9:
                stop_flag = 1
            ...
            if sign_i == 0:
                integral_v_l, in_v_last_l, v_out_l = v_pid(real_pur_v, v_l, 1.2* kp_v_l, 85 * ki_v_l, kd_v_l, integral_v_l, in_v_last_l, inte_v_limit)
                integral_v_r, in_v_last_r, v_out_r = v_pid(real_pur_v, v_r, 1.2* kp_v_r, 85 * ki_v_r, kd_v_r, integral_v_r, in_v_last_r, inte_v_limit)
            else:
