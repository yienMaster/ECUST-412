...
def time_pit_handler_1(time):
    ...
    if run_flag:
        ...
        if ticker_count % 10 == 0:
            ...
            if abs(v_l) >= pur_v + 50 or abs(w_z_real) > 15.5:
                stop_flag = 1
            ...
            if v_l - real_pur_v >= -60:
                sign_i = 1
                
            if sign_i == 0:
                integral_v_l, in_v_last_l, v_out_l = v_pid(real_pur_v, v_l, 1.6* kp_v_l, 90 * ki_v_l, kd_v_l, integral_v_l, in_v_last_l, inte_v_limit)
                integral_v_r, in_v_last_r, v_out_r = v_pid(real_pur_v, v_r, 1.6* kp_v_r, 90 * ki_v_r, kd_v_r, integral_v_r, in_v_last_r, inte_v_limit)
            else:
