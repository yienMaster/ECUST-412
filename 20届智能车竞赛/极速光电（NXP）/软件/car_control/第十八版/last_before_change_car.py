...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.1 * v_real ** 2 + 0.4 * v_pur ** 2) * kp * offset + 4500 * gyro +d_corrcte * kd * (in_now - in_last)
...
def time_pit_handler_1(time):
    ...
    if run_flag:
        ...
        if ticker_count % 10 == 0:
            ...
            if sign_i == 0:
                integral_v_l, in_v_last_l, v_out_l = v_pid(pur_v, v_l, 1* kp_v_l, 60 * ki_v_l, kd_v_l, integral_v_l, in_v_last_l, inte_v_limit)
                integral_v_r, in_v_last_r, v_out_r = v_pid(pur_v, v_r, 1* kp_v_r, 60 * ki_v_r, kd_v_r, integral_v_r, in_v_last_r, inte_v_limit)
            else:
