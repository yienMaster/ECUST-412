...
def follow_line(pur, data, gyro , kp, kd, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 6900 + in_now * 0.075
...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.3 * v_real ** 2 + 0.2 * v_pur ** 2) * kp * offset + 9100 * gyro +d_corrcte * kd * (in_now - in_last)
...
while True:
    ...
    if cal_flag :
        ...
        #wireless.send_oscilloscope(mode_4, mode_2, left_m, right_m, left_f, right_f)
