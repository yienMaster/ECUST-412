...
pit = ticker(1)
# 关联采集接口
pit.capture_list(encoder_l, encoder_r,imu,tof,ccd)
# 关联 Python 回调函数
...
def ccd_road_find(ccd_data):
    ...
    if len(boundary_points) == 2:
        ...
        return None, None, None, 0  # 如果没有找到交界点，返回None和0
...
def follow_road(line_pos, w_z_real, v_l, v_r, theita_real, w_y_real):
    ...
    integral_w_y_r, pid_out_r = w_y_pid(w_y_pur_r, w_y_real, kp_w_y_r, ki_w_y_r, integral_w_y_r ,inte_w_y_limit)
...
while True:
    ...
    if mode_1 != 3:
        ...
        if sign_cro == 0:
            ...
            if sign_cro <= 2:
                sign_cro += 1         
        elif abs(road_width_f - total_road_width_f) >= 10:
            sign_cro = 0
        
        if sign_cro == 3:
            if abs(road_width_f - total_road_width_f) >= 60:
                sign_cro = 0
                mode_1 = 2
                mode_2 = 1
    ...
    if mode_1 == 1:
        ...
        if mode_2 == 1:
            if sign_cro_out == 0:
                ...
                if abs(road_width_n - total_road_width_n) >= 30:
                    sign_cro_out = 0
                    mode_1 = 1
                    mode_2 = mode_pre
