...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.3 * v_real ** 2 + 0.2 * v_pur ** 2) * kp * offset + 8600 * gyro +d_corrcte * kd * (in_now - in_last)
...
def time_pit_handler_1(time):
    ...
    if run_flag:
        ...
        if ticker_count % 10 == 0:
            ...
            if sign_i == 0:
                integral_v_l, in_v_last_l, v_out_l = v_pid(real_pur_v, v_l, 1.4 * kp_v_l, 30 * ki_v_l, kd_v_l, integral_v_l, in_v_last_l, inte_v_limit)
                integral_v_r, in_v_last_r, v_out_r = v_pid(real_pur_v, v_r, 1.4 * kp_v_r, 30 * ki_v_r, kd_v_r, integral_v_r, in_v_last_r, inte_v_limit)
            else:
                if v_l < 30:
        ...
        if(stop_flag == 0 and stop_sign != 3 and v_l >= 0):
...
def time_pit_handler_2(time):
    ...
    '''
    global key_flag
    global key_count, run_flag, choose_flag, sta_1, sta_2, sta_3,sta_4
    key_flag = True 
    ...
    if key_count == 100:
        ...
        if key_data[0]:
            ...
            key.clear(1)
        if key_data[1]:
            print("run_flag = {:>6d}.".format(run_flag))
            print("choose_flag = {:>6d}.".format(choose_flag))
            print("sta_1 = {:>6d}.".format(sta_1))
            print("sta_2 = {:>6d}.".format(sta_2))
            print("sta_3 = {:>6d}.".format(sta_3))
        if key_data[2]:
            if sta_4 != 0:
                choose_flag = (choose_flag + 1) if (choose_flag < 4) else (1)
            key.clear(3)
        if key_data[3]:
            if choose_flag == 1:
...
while True:
    ...
    if cal_flag :
        ...
        #wireless.send_oscilloscope(mode_1, mode_2, mode_3, mode_4,right_f )
        ...
        if mode_1 == 1:
            ...
            #wireless.send_oscilloscope(stop_sign,theita_cal,road_width_f_now,road_width_m_now)
            if stop_sign == 0:
                if stop_num == 0:
        ...
        if mode_1 == 1:
            ...
            if mode_2 == 1:
                ...
                if mode_3 == 0:
                    ...
                    if sta_2 == 1 and (((left_f - last_left_f)<= -15) and left_f == -53 and abs(right_f-road_width_f/2)<6):
                        sign_sci_l = 1

                    elif sta_3 == 1 and ( right_f >= 50 and abs(left_f+road_width_f/2)<10):
                        sign_sci_r = 1
                    ...
                    if mode_4 == 0:
                        ...
                        if  (abs(road_width_m_now - 90) <= 15 and abs(left_m+right_m) <=8) or (abs(road_width_f_now - road_width_f) <= 10 and abs(left_f+right_f) <=5):
                    ...
                    if mode_4 != 3 and mode_4 != 4 and mode_4 != 5:
                        ...
                        if sign_sci_m == 0:
                            if left_m == -53:
                                right_m = left_m + road_width_m*1.35
                            else:
                                right_m = left_m + road_width_m*1.15
                        else:
                            if right_m == 53:
                                left_m = right_m - road_width_m*0.7
                            else:
                                left_m = right_m - road_width_m*0.8
                        
                        if sign_sci_f == 0:
                            if left_f == -53:
                                right_f = left_f + road_width_f*1.25
                            else:
                                right_f = left_f + road_width_f*1.15
                        else:
                            if right_f == 53:
                                left_f = right_f - road_width_f*0.7
                            else:
                                left_f = right_f - road_width_f*0.8
                        ...
                        #right_m = left_m + road_width_m * 1.4

                        #right_f = left_f + road_width_f * 1.4
                    ...
                    if mode_4 != 4:
                        line_pos = (m1 * (left_m + right_m) + f1 * (left_f + right_f))*(1/2)
                        
                        if mode_4 == 3 and line_pos > -10:
                            line_pos = -15
                    ...
                    if mode_4 == 0:
                        ...
                        if (abs(road_width_m_now - road_width_m) <= 15 and abs(left_m+right_m) <=8) or (abs(road_width_f_now - road_width_f) <= 15 and abs(left_f+right_f) <=5):
                    ...
                    if mode_4 != 3 and mode_4 != 4 and mode_4 != 5:
                        ...
                        if sign_sci_m == 1:
                            if left_m == -53:
                                right_m = left_m + road_width_m*0.8
                            else:
                                right_m = left_m + road_width_m*1.1
                        else:
                            if right_m == 53:
                                left_m = right_m - road_width_m*1.35
                            else:
                                left_m = right_m - road_width_m*1.15
                        
                        if sign_sci_f == 1:
                            if left_f == -53:
                                right_f = left_f + road_width_f*0.8
                            else:
                                right_f = left_f + road_width_f*0.9
                        else:
                            if right_f == 53:
                                left_f = right_f - road_width_f*1.25
                            else:
                                left_f = right_f - road_width_f*1.15
    ...
    '''
        ...
        if sta_4 == 1:
            ...
            if sta_4 == 1:
                lcd.str32(15, 120, "SCREEN    =    ON ", 0xFFFF)
            else:
                lcd.str32(15, 120,  "SCREEN    =   OFF", 0xFFFF)
    '''
