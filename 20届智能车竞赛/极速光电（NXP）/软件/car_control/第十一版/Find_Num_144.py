...
def follow_road(line_pos,  w_z_real , v_l, v_r, theita_real, w_y_real):
    ...
    #wireless.send_oscilloscope(pur_theita , kp_1, kd_1, kp_2, kd_2, line_pos)
...
while True:
    ...
    if cal_flag :
        ...
        #75 60
        left_n, right_n, width_n = find_road(ccd_data1 , 80)
        #print(left_n, right_n)
        left_m, right_m, width_m = find_road(ccd_data2 , 63)
        left_f, right_f, width_f = find_road(ccd_data3 , 41)
        ...
        line_pos = (0.4 * (left_n + right_n) + 0.5 * (left_m + right_m) + 0.1 * (left_f + right_f))*(1/4)        
        #位姿解算
        mx = imu_data[6] + 3216
        my = imu_data[7] - 24 
        mz = imu_data[8] + 1181
