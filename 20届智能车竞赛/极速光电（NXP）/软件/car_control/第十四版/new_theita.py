...
#电机初始化
motor_l = MOTOR_CONTROLLER(MOTOR_CONTROLLER.PWM_C30_DIR_C31, 17000, duty = 0, invert = False)
motor_r = MOTOR_CONTROLLER(MOTOR_CONTROLLER.PWM_C28_DIR_C29, 17000, duty = 0, invert = True)
...
def time_pit_handler_1(time):
    global x, p, ticker_count, v_l, v_r, cal_flag ,theita_1, theita_2, w_y, theita, w_y_last, theita_1_last
    global stop_flag,theita_2_flag, theita_2_cre , w_z
    global in_dis_last, v_turn, w_z_cal, line_pos
    ...
    if theita_2_flag >= 75 and (theita_1 < 2.5 and theita_1 > 1.7) and (abs(theita_1 - theita_2) < 0.2):
        theita_2 = theita_1
        theita_2_flag = 0
    else:
    ...
    if abs(theita_1 - theita_2) > 1:
        theita_1 = theita_2
     '''
    theita_2 = theita_1_last +  0.044*(0.8 * w_y_last + 0.2 * w_y)
    theita = theita_1 + 0.5 * (theita_2 - theita_1)
        
    theita_1_last = theita_1
    w_y_last = w_y
    #卡尔曼滤波
    #z = [theita,w_y]
    ...
    wireless.send_oscilloscope(theita, theita_1, theita_2, w_y)
    ...
    if run_flag:
        ...
        if ticker_count % 2 == 0:
            ...
            in_theita_last_l, pur_w_y_l = theita_pid(pur_theita_l, theita, kp_theita_l, kd_theita_l, in_theita_last_l)
            in_theita_last_r, pur_w_y_r = theita_pid(pur_theita_r, theita, kp_theita_r, kd_theita_r, in_theita_last_r)
            pur_w_y_l = -pur_w_y_l
            pur_w_y_r = -pur_w_y_r
        ...
        integral_w_y_l,in_w_y_last_l, w_y_out_l = w_y_pid(pur_w_y_l, w_y, kp_w_y_l, ki_w_y_l, kd_w_y_l, integral_w_y_l ,in_w_y_last_l, inte_w_y_limit)
        integral_w_y_r,in_w_y_last_r, w_y_out_r = w_y_pid(pur_w_y_r, w_y, kp_w_y_r, ki_w_y_r, kd_w_y_r, integral_w_y_r ,in_w_y_last_r, inte_w_y_limit)
        w_y_out_l = -w_y_out_l
        w_y_out_r = -w_y_out_r
...
pit2.capture_list(encoder_l, encoder_r, ccd)
