...
def follow_line(pur, data, gyro , kp, kd, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 500 + in_now * 0.2
...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = kp * offset + d_corrcte * kd * (in_now - in_last)
...
while True:
    ...
    if cal_flag :
        ...
        if mode_1 == 1:
            #直行状态
            if mode_2 == 1:
                line_pos = (0.6 * (left_n + right_n) + 0.3 * (left_m + right_m) + 0.1 * (left_f + right_f))*(1/2) 
