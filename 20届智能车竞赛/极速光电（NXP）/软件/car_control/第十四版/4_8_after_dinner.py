...
def stand( theita_l, theita_r, v_l, v_r, theita_real, w_y_real, turn):
    '''
    直立环
    '''
    global stop_flag
    ...
    if(stop_flag == 0):
        ...
        if turn > 0:
            motor_duty_l = motor_duty_l + 0.7 * turn
            motor_duty_r = motor_duty_r - 1 * turn
        else:
            motor_duty_l = motor_duty_l + 1 * turn
            motor_duty_r = motor_duty_r - 0.7 * turn
        
        motor_duty_l = motor_duty_l + 120
...
def follow_road(line_pos,  w_z_real):
    '''
    pid车辆循迹
    '''
    global in_dis_last
...
def run(v_l, v_r):
    ...
    integral_v_l, in_v_last_l, pid_out_l = v_pid(pur_v, v_l, kp_v_l, ki_v_l, kd_v_l, integral_v_l, in_v_last_l, inte_v_limit)
    integral_v_r, in_v_last_r, pid_out_r = v_pid(pur_v, v_r, kp_v_r, ki_v_r, kd_v_r, integral_v_r, in_v_last_r, inte_v_limit)
...
def time_pit_handler_1(time):
    global x, p, ticker_count, v_l, v_r, v_turn, turn_ticker
    global run_l, run_r
    ...
    turn_ticker = (turn_ticker + 1) if (turn_ticker < 2) else (1)
    
    turn_add = v_turn * turn_ticker / 2
    ...
    theita_add_l = ticker_count * run_l / 4
    theita_add_r = ticker_count * run_r / 4
    ...
    #wireless.send_oscilloscope(turn_add,theita_add_l )
    if run_flag:
        #stand(theita_add_l, theita_add_r, v_l, v_r, theita_cal, w_y_cal, 0)
        stand(theita_add_l, theita_add_r, v_l, v_r, theita_cal, w_y_cal, v_turn)
        
        
def time_pit_handler_2(time):
    global line_pos, w_z_cal, v_turn, cal_flag
    
    v_turn = follow_road(line_pos,  w_z_cal)
    
    cal_flag = 1
    
    
def time_pit_handler_3(time):
    global v_l, v_r
    global run_l, run_r
    
    run_l, run_r = run(v_l, v_r)
...
pit3.callback(time_pit_handler_3)
...
while True:
    ...
    if cal_flag :
        ...
        #75 60
        left_n, right_n, road_width_n = find_road(ccd_data1 , straight_road_width_n)
        #wireless.send_oscilloscope(road_width_n,mode_1,sign_err)
        #print(left_n, right_n)
        left_m, right_m, road_width_m = find_road(ccd_data2 , straight_road_width_m)
        left_f, right_f, road_width_f = find_road(ccd_data3 , straight_road_width_f)
        ...
        if mode_1 == 1:
            #直行状态
            if mode_2 == 1:
                line_pos = (0.17 * (left_n + right_n) + 0.38 * (left_m + right_m) + 0.45 * (left_f + right_f))*(1/2) 
