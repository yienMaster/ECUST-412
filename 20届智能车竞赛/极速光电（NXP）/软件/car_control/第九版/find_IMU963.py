...
while True:
    if (ticker_flag and ticker_count % 100 == 0):
        ...
        '''
        for i in range(100):  # 从0到127
            if(i=100):
                processed_data1.pop(0)
                processed_data1.append(imu_data[6])
        average1 = sum(processed_data1) / 100
        ...
        #偏航角
        b = imu_data[7] * math.cos(theita) - imu_data[8] * math.sin(theita)
        if(imu_data[6] == 0):
            c = 0.0001
        else:
            c = imu_data[6] 
