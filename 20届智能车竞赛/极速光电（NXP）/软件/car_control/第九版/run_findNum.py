...
def v_pid(pur, data, kp, ki, kd, inte, v_limit, in_last):
    '''
    PID 控制直立
...
def follow_road(line_pos,v_l, v_r, theita_real, w_y_real):
    ...
    global in_theita_last_l, in_theita_last_r, integral_w_y_l, integral_w_y_r, in_v_last_l, in_v_last_r
    '''转向环'''
    ...
    in_v_last_l, integral_v_l, pid_out_l = v_pid(pur_v_l, v_l, kp_v_l, ki_v_l, kd_v_l, integral_v_l, inte_v_limit, in_v_last_l)
    in_v_last_r, integral_v_r, pid_out_r = v_pid(pur_v_r, v_r, kp_v_r, ki_v_r, kd_v_r, integral_v_r, inte_v_limit, in_v_last_r)
    
    pur_theita_l = pur_theita + pid_out_l
    pur_theita_r = pur_theita + pid_out_r
    
    #pur_theita_l = theita_real - pid_out_l
    #pur_theita_r = theita_real - pid_out_r
    ...
    wireless.send_oscilloscope(motor_duty_l,v_l, pur_v , kp_v_l * 100000 ,ki_v_l * 10000000 ,kd_v_l * 100000 ,integral_v_l)
...
def time_pit_handler_1(time):
    ...
    global ticker_count ,v_l_last, v_r_last, v_l_now ,v_r_now
    
    ticker_count = (ticker_count + 1) if (ticker_count < 5) else (1)
    
    if ticker_count == 5:
        v_l_now = encoder_l.get() 
        v_r_now = encoder_r.get()
        
        v_l = (v_l_last + v_l_now)/ 2
        v_r = (v_l_last + v_l_now)/ 2
        v_l_last = v_l_now
        v_r_last = v_r_now
        cal_flag = True
