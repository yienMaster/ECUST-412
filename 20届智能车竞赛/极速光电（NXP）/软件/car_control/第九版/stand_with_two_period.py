...
def find_road(data):
    ...
    for i in range(12, n - 11):
        ...
        # 计算差比和
        diff_ratio = abs(left - right) / (center + 1)  # 避免除零
        filtered_data[i] = int(diff_ratio * 255)  # 归一化到 0-255 范围
...
def run( v_turn ,v_l, v_r, theita_real, w_y_real):
    ...
    #wireless.send_oscilloscope(motor_duty_l,v_l, pur_v , kp_v_l * 100000 ,ki_v_l * 10000000 ,integral_v_l)
...
while True:
    ...
    if cal_flag :
        ...
        left_n, right_n, width_n = find_road(ccd_data1)
        #left_f, right_f, width_f = find_road(ccd_data2)
        ...
        #卡尔曼滤波
        z = [omiga, w_z]
        x_o, p_o = kalman_filter_explicit(x_o, p_o, z, F, H, Q, R)
        
        omiga_cal =  x_o[0]
        w_z_cal = x_o[1] - 0.000327907906991503
