...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.3 * v_real ** 2 + 0.2 * v_pur ** 2) * kp * offset + 7600 * gyro +d_corrcte * kd * (in_now - in_last)
...
def time_pit_handler_1(time):
    ...
    if run_flag:
        ...
        if(stop_flag == 0 and stop_sign != 2):
...
while True:
    ...
    if cal_flag :
        ...
        wireless.send_oscilloscope(stop_flag, stop_sign)
        ...
        if mode_1 == 1:
            ...
            if mode_2 == 1:
                ...
                if abs((left_m + right_m) - (left_f + right_f)) >= 25:
                    line_pos = left_m + right_m
