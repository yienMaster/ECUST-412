...
def dis_pid(pur, data, kp, kd, in_last):
    ...
    return in_now, out
'''
def w_z_pid(pur, data, kp, kd, in_last):
...
def follow_line(pur, data, gyro , kp, kd, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 10000 + in_now * 0.15
...
#    out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + (0.2 * v_real ** 2 + 0.8 * v_pur ** 2)) * kp * offset + gyro * 4200 +d_corrcte * kd * (in_now - in_last)
    #out = kp * offset + gyro * 2000 + d_corrcte * kd * (in_now - in_last)
    if v_pur != 0:
        out = out * v_real / v_pur
...
def time_pit_handler_1(time):
    global x, p, ticker_count, v_l, v_r, cal_flag ,theita_1, theita_2, w_y, theita, w_y_last, w_z
    global stop_flag, w_z_real, theita_1_last, turn_count, v_turn_last,w_z_pur, in_w_z_last
    global in_dis_last, v_turn, line_pos, d_corrcte_last,curvature , turn_sign
    ...
    w_z = 10 * w_z_dps * trans * 0.7 + 0.3 * w_z
    #wireless.send_oscilloscope(w_z)
    ...
    if run_flag:
        if turn_count == 3:
            ...
            w_z_pur = 0.7 * (v_l + v_r) / 2 * curvature / 60 + 0.3 * w_z_pur
        ...
        in_w_z_last, v_turn = w_z_pid(w_z_pur, w_z, kp_w_z, kd_w_z ,in_w_z_last)
        '''
        if abs(v_turn_last - v_turn) <= 800:
            v_turn_last = v_turn
        elif abs(v_turn_last - v_turn) <= 1500:
            v_turn = 0.3 * v_turn_last + 0.7 * v_turn
            v_turn_last = v_turn
        elif abs(v_turn_last - v_turn) <= 2500:
            ...
            '''
            '''
        if abs(v_turn_last - v_turn) >= 2500:
            v_turn_last = v_turn
        else:
            v_turn = 0.1 * v_turn_last + 0.9 * v_turn
            v_turn_last = v_turn
'''
        wireless.send_oscilloscope(v_turn, w_z_pur, w_z , curvature )
        ...
        #motor_duty_l = w_y_out_l * (1 + v_turn / 2500)
        #motor_duty_r = w_y_out_r * (1 - v_turn / 2000)
        
        if(motor_duty_l >= motor_duty_max):
            motor_duty_l = motor_duty_max0
...
while True:
    ...
    if cal_flag :
        ...
        road_mid_m = (left_m + right_m) / 2
        road_mid_f = (left_f + right_f) / 2
        ...
        if m == 0:
            ...
            x_R =  ((- x1 * x1 / (2 * y1) + (x2 * x2 - x1 * x1) / (2 * (y2 - y1)) + y2 / 2)
                            / m)
            y_R = - (x1 / y1) * x_R + x1 * x1 / (2 * y1) + y1 / 2
            R_turn_2 = x_R * x_R + y_R * y_R
        curvature = 0.7 * FindCurvature(R_turn_2) + 0.3 * curvature
        #wireless.send_oscilloscope(curvature,R_turn_2)
        '''
        if sign_err == 0:   
            if road_width_n == 0:
                sign_err = 1    
