...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.1 * v_real ** 2 + 0.4 * v_pur ** 2) * kp * offset + 5700 * gyro +d_corrcte * kd * (in_now - in_last)
...
def time_pit_handler_1(time):
    ...
    if run_flag:
        ...
        if ticker_count % 10 == 0:
            ...
            if sign_i == 0:
                integral_v_l, in_v_last_l, v_out_l = v_pid(pur_v, v_l, 1* kp_v_l, 3.8 * ki_v_l, kd_v_l, integral_v_l, in_v_last_l, inte_v_limit)
                integral_v_r, in_v_last_r, v_out_r = v_pid(pur_v, v_r, 1* kp_v_r, 3.8 * ki_v_r, kd_v_r, integral_v_r, in_v_last_r, inte_v_limit)
            else:
...
while True:
    ...
    if cal_flag :
        ...
        if road_width_n_now == 106 or road_width_n_now == 0:
            if abs(last_left_n + last_right_n) >= 45:
                left_n = last_left_n
        ...
        if road_width_m_now == 106 or road_width_m_now == 0:
            if abs(last_left_m + last_right_m) >= 45:
                left_m = last_left_m
        ...
        if road_width_f_now == 106 or road_width_f_now == 0:
            if abs(last_left_f + last_right_f) >= 45:
                left_f = last_left_f
        ...
        if abs(road_width_f_now - 50 ) < 20:
            road_width_f = road_width_f_now
        
        if abs(road_width_m_now - 70) < 20:
            road_width_m = road_width_m_now
            
        if abs(road_width_n_now - 85) < 20:
