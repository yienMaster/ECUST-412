...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.1 * v_real ** 2 + 0.4 * v_pur ** 2) * kp * offset + 5500 * gyro +d_corrcte * kd * (in_now - in_last)
...
def time_pit_handler_1(time):
    ...
    if run_flag:
        ...
        if ticker_count % 10 == 0:
            ...
            if sign_i == 0:
                integral_v_l, in_v_last_l, v_out_l = v_pid(pur_v, v_l, 1* kp_v_l, 5.8 * ki_v_l, kd_v_l, integral_v_l, in_v_last_l, inte_v_limit)
                integral_v_r, in_v_last_r, v_out_r = v_pid(pur_v, v_r, 1* kp_v_r, 5.8 * ki_v_r, kd_v_r, integral_v_r, in_v_last_r, inte_v_limit)
            else:
...
straight_road_width_n = 62
straight_road_width_m = 52
straight_road_width_f = 34
    ...
    if cal_flag :
        ...
        if mode_1 == 1:
            ...
            if sign_sci == 0:
                ...
                sci_road_width_n_out = 0.5 * road_width_n + 0.43 * total_road_width
                sci_road_width_m_out = 0.5 * road_width_m + 0.43 * total_road_width
                sci_road_width_f_out = 0.5 * road_width_f + 0.43 * total_road_width
                
                if (abs(road_width_f_now - sci_road_width_f_in) <= 5):
                    sign_sci = 1
                    
                    if (((left_f - last_left_f)<= -15) and left_f <= -48 and (abs(right_f - last_right_f) <= 3)):
                        sign_sci_l = 1
                    elif (((right_f - last_right_f) >= 15) and right_f >= 48 and (abs(left_f - last_left_f) <= 3)):
                        sign_sci_r = 1
                    else:
                        sign_sci = 0
        ...
        if mode_1 == 1:
            ...
            if mode_2 == 1:
                ...
                if abs(road_width_f_now - 34 ) < 10:
                    road_width_f = road_width_f_now
                
                if abs(road_width_m_now - 52) < 10:
                    road_width_m = road_width_m_now
                    
                if abs(road_width_n_now - 62) < 10:
            ...
            if mode_2 == 1:
                ...
                if mode_3 == 1:
                    ...
                    if mode_4 == 0:
                        if abs(road_width_n_now - sci_road_width_n_in) <= 3:
                            sci_num += 1
                            if sci_num == 2:
                                sci_num = 0
                                mode_4 = 1
                        ...
                        if sign_sci_f == 1:
                            ...
                            if sign_sci_m == 1:
                                ...
                                if sign_sci_n == 1:
                                    if abs(road_width_n_now - road_width_n) <= 5:
                                        sci_num += 1
                                        if sci_num == 2:
                                            sci_num = 0
                                            sign_sci_n = 0
                        ...
                        if sci_num <= 2:
                            if road_width_n_now >= road_width_n * 1.5:
                                sci_num += 1
                                
                        elif sci_num >= 3:
                            if road_width_n <= road_width_n * 1.2:
                                sci_num += 1
                                if sci_num == 5:
                                    sci_num = 0
                                    sign_sci_n = 1
                        ...
                        if sign_sci_f == 1:
                            ...
                            if sci_num == 0:
                                ...
                                if road_width_f_now - sci_road_width_f_out >= -5:
                                    sci_num += 1
                            ...
                            if sign_sci_m == 1:
                                ...
                                if sci_num == 0:
                                    if (right_m - last_right_m >= 15):
                                        sci_num += 1
                                else:
                                    if road_width_m_now - sci_road_width_m_out >= -5:
                                        sci_num += 1
                                ...
                                if sign_sci_n == 1:
                                    if sci_num == 0:
                                        if (right_n - last_right_n >= 15):
                                            sci_num += 1
                                    else:
                                        if road_width_n_now - sci_road_width_n_out >= -5:
                                            sci_num += 1
        ...
        wireless.send_oscilloscope(left_n, right_n, left_m, right_m, left_f, right_f, mode_1,mode_4);
