...
def follow_line(pur, data, gyro , kp, kd, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 11000 + in_now * 0.08
...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.1 * v_real ** 2 + 0.4 * v_pur ** 2) * kp * offset + 5300 * gyro +d_corrcte * kd * (in_now - in_last)
