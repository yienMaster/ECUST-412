...
def follow_line(pur, data, gyro , kp, kd, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 6000 + in_now * 0.06
...
def time_pit_handler_1(time):
    ...
    global in_dis_last, v_turn, line_pos, d_corrcte_last,v_turn_last, sign_i
    ...
    if run_flag:
        if turn_count == 3:
            ...
            if abs(v_turn_last - v_turn) <= 800:
                ...
                v_turn_last = v_turn
            elif abs(v_turn_last - v_turn) <= 1800:
