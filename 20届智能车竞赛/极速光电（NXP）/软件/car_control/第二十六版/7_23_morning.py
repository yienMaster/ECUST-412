...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.3 * v_real ** 2 + 0.2 * v_pur ** 2) * kp * offset + 9000 * gyro +d_corrcte * kd * (in_now - in_last)
        ...
        if ticker_count % 10 == 0:
            ...
            if sign_i == 0:
                ...
                if v_l <= 30:
                    ...
                    integral_v_l, in_v_last_l, v_out_l = v_pid(real_pur_v, v_l, 2 * kp_v_l, 25 * ki_v_l, kd_v_l, integral_v_l, in_v_last_l, inte_v_limit)
                    integral_v_r, in_v_last_r, v_out_r = v_pid(real_pur_v, v_r, 2 * kp_v_r, 25 * ki_v_r, kd_v_r, integral_v_r, in_v_last_r, inte_v_limit)
