...
#电机初始化
motor_l = MOTOR_CONTROLLER(MOTOR_CONTROLLER.PWM_C30_DIR_C31, 24000, duty = 0, invert = False)
motor_r = MOTOR_CONTROLLER(MOTOR_CONTROLLER.PWM_C28_DIR_C29, 24000, duty = 0, invert = True)
...
def find_road(data, width):
    ...
    if find_l == 0 and find_r == 0:
        if left_edge == 64 and right_edge == 64:
            if data[70] > 0 and data[74] > 0 and data[80] > 0:
                left_edge = -53 + 64
                right_edge = 53 + 64
            else:
                left_edge =  64
...
def run( v_turn ,v_l, v_r, theita_real, w_y_real):
    ...
    直立环
    '''
    global integral_v_l, integral_v_r , in_v_last_l, in_v_last_r, stop_flag
    ...
    #更新电机PWM输出
    #wireless.send_oscilloscope(motor_duty_r,kp_theita_l,kd_theita_l,kp_w_y_l,ki_w_y_l,pur_theita,theita_real,integral_w_y_l)
    ...
    #wireless.send_oscilloscope(v_l, v_r, pur_v, integral_v_l)
...
def time_pit_handler_1(time):
    ...
    global cal_flag, x, p
    global ticker_count, w_z_cal, inte_flag, del_flag
    ticker_count = (ticker_count + 1) if (ticker_count < 2) else (1)
    inte_flag = (inte_flag + 1) if (inte_flag < 50) else (0)
...
sci_road_width_n_out = 0.5 * straight_road_width_n + 0.4 * total_road_width
sci_road_width_m_out = 0.5 * straight_road_width_m + 0.4 * total_road_width
sci_road_width_f_out = 0.5 * straight_road_width_f + 0.4 * total_road_width
...
bar_road_width_f = 0.75 * straight_road_width_f
bar_road_width_m = 0.75 * straight_road_width_m
bar_road_width_n = 0.75 * straight_road_width_n
...
while True:
    ...
    if cal_flag :
        ...
        #75 60
        left_n, right_n, road_width_n = find_road(ccd_data1 , 83)
        #print(left_n, right_n)
        left_m, right_m, road_width_m = find_road(ccd_data2 , 67)
        left_f, right_f, road_width_f = find_road(ccd_data3 , 51)
        ...
        if mode_1 == 1:
            ...
            if sign_sci == 0:
                if (abs(road_width_f - sci_road_width_f_in) <= 5):
                    sign_sci = 1
                     ...
                     if (((left_f - last_left_f) + (sci_road_width_f_in - straight_road_width_f) <= 3) and left_f <= -48  and (abs(right_f - last_right_f) <= 3)):
                        sign_sci_l = 1
                    elif (((right_f - last_right_f) - (sci_road_width_f_in - straight_road_width_f) <= 3) and right_f >= 48 and (abs(left_f - last_left_f) <= 3)):
                        sign_sci_r = 1
                    else:
                        sign_sci = 0
            ...
            if sign_cro == 0:
             ...
             #障碍物检测
            if sign_bar == 0 :
                if (abs(road_width_f - bar_road_width_f) <= 3) :
                    sign_bar = 1
                ...
                if (abs(road_width_f - road_width_bar) <= 3) :
                    sign_bar += 1
                elif abs(road_width_f - total_road_width_f) <= 3:
                    sign_bar = 0
            else:
                if abs(road_width_f - total_road_width_f) <= 3:
                    sign_bar = 0
                    mode_1 = 2
                    mode_2 = 3
                    if sign_bar_l == 1:
        ...
        if mode_1 == 1:
            #直行状态
            if mode_2 == 1:
                line_pos = (0.38 * (left_n + right_n) + 0.47 * (left_m + right_m) + 0.15 * (left_f + right_f))*(1/2) 
            ...
            if mode_2 == 1:
                ...
                if mode_3 == 1:
                    ...
                    if mode_4 == 0:
                        ...
                        if sci_num <= 2:
                            if road_width_n >= straight_road_width_n + 10:
                                sci_num += 1
                                
                        elif sci_num >= 3:
                            if road_width_n <= straight_road_width_n + 5:
                                sci_num += 1
                                if sci_num == 5:
                                    sci_num = 0
                                    sign_sci_n = 1
                        ...
                        if sign_sci_f == 1:
                            ...
                            if sci_num == 0:
                                ...
                                if abs(sci_road_width_f_out - road_width_f) <= 5:
                                    sci_num += 1
                            ...
                            if sign_sci_m == 1:
                                ...
                                if sci_num == 0:
                                    ...
                                    if abs(sci_road_width_m_out - road_width_m) <= 5:
                                        sci_num += 1
                                ...
                                if sign_sci_n == 1:
                                    if sci_num == 0:
                                        ...
                                        if abs(sci_road_width_n_out - road_width_n) <= 5:
                                            sci_num += 1
                        ...
                        if sign_sci_f == 0:
                            ...
                            if sign_sci_m == 0:
                                ...
                                if abs(road_width_m - straight_road_width_m) <= 4:
                                    sci_num += 1
                                    if sci_num == 2:
                                        sci_num = 0
                                        sign_sci_m = 1
                    ...
                    line_pos = (0.48 * (left_n + right_n) + 0.47 * (left_m + right_m) + 0.05 * (left_f + right_f))*(1/2) 
                ...
                if abs(road_width_m - bar_road_width_m) <= 3:
                    sign_bar_m = 1
                ...
                if sign_bar_out and sign_bar_n == 0:
                 line_pos = (0.38 * (left_n + right_n) * sign_cro_n  + 0.47 * (left_m + right_m) * sign_cro_m + 0.15 * (left_f + right_f) * sign_cro_f)*(1/2) 
