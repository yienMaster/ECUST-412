...
def find_road(data, width, last_left, last_right):
    """
    ...
    for i in range(0, 53):
        ...
        left_point_l = last_left - i
        
        left_point_r = last_left + i
        
        if find_l == 0:
            if 64 >= left_point_l >= 11:
                l = data[left_point_l - 1]
                c = data[left_point_l]
                r = data[left_point_l + 1]
                ...
                if abs(diff_ratio) > 75:
                    if diff_ratio > 0:
                        if find_r == 0 :
                            right_edge = left_point_l
                    if diff_ratio < 0:
                        left_edge = left_point_l
                        find_l = 1
            
            if 11 <= left_point_r <= 64:
                
                l = data[left_point_r - 1]
                c = data[left_point_r]
                r = data[left_point_r + 1]
                ...
                if abs(diff_ratio) > 75:
                    if diff_ratio > 0:
                        if find_r == 0 :
                            right_edge = left_point_r
                    if diff_ratio < 0:
                        left_edge = left_point_r
                        find_l = 1
        
        right_point_l = last_right - i
        
        right_point_r = last_right + i
        
        if(find_r == 0):
            if right_point_l >= 64 and right_point_l <= 107:
                l = data[right_point_l - 1]
                c = data[right_point_l]
                r = data[right_point_l + 1]
                ...
                if abs(diff_ratio) > 75:
                    if diff_ratio > 0:
                        right_edge = right_point_l
                        find_r = 1
                    if diff_ratio < 0:
                        if find_l == 0:
                            left_edge = right_point_l
                            
            if right_point_r <= 107 and right_point_r >= 64:
                l = data[right_point_r - 1]
                c = data[right_point_r]
                r = data[right_point_r + 1]
                ...
                if abs(diff_ratio) > 75:
                    if diff_ratio > 0:
                        right_edge = right_point_r
                        find_r = 1
                    if diff_ratio < 0:
                        if find_l == 0:
                            left_edge = right_point_r
...
def run( v_turn ,v_l, v_r, theita_real, w_y_real):
    ...
    if k >= 0:
        pur_v_l = pur_v * (1 - k)
        pur_v_r = pur_v * (1 + 0.5 * k)
    else:
        pur_v_l = pur_v * (1 - 0.5 * k)
        pur_v_r = pur_v * (1 + k)
        '''
...
def time_pit_handler_1(time):
    global v_l, v_r ,theita_cal, w_y_cal, line_pos, v_turn
    global cal_flag, x, p
...
while True:
    ...
    if cal_flag :
        ...
        #75 60
        left_n, right_n, road_width_n = find_road(ccd_data1 , 83, last_left_n + 64, last_right_n + 64)
        #wireless.send_oscilloscope(road_width_n,mode_1,sign_err)
        #print(left_n, right_n)
        left_m, right_m, road_width_m = find_road(ccd_data2 , 66, last_left_m + 64, last_right_m + 64)
        left_f, right_f, road_width_f = find_road(ccd_data3 , 52, last_left_f + 64, last_right_f + 64)
        ...
        if mode_1 == 1:
            #直行状态
            if mode_2 == 1:
                line_pos = (0.08 * (left_n + right_n) + 0.47 * (left_m + right_m) + 0.45 * (left_f + right_f))*(1/2) 
            ...
            if mode_2 == 1:
                ...
                if mode_3 == 1:
                    ...
                    line_pos = (0.6 * (left_n + right_n) + 0.4 * (left_m + right_m) + 0.00 * (left_f + right_f))*(1/2)
                #elif mode_3 == 2:
        ...
        #wireless.send_oscilloscope(left_n, right_n, left_m, right_m, left_f, right_f)
        #位姿解算
        mx = imu_data[6] + 3216
        my = imu_data[7] - 24 
        mz = imu_data[8] + 1181
    ...
    '''
#if data_wave[0] != 0:
    kp_v_l = data_wave[4] * 0.00001
    kp_v_r = kp_v_l
         
#if data_wave[1] != 0:
    ki_v_l = data_wave[5] * 0.00000005
    ki_v_r = ki_v_l
    '''
#if data_wave[2] != 0:
