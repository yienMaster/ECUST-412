...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.3 * v_real ** 2 + 0.2 * v_pur ** 2) * kp * offset + 8900 * gyro +d_corrcte * kd * (in_now - in_last)
...
def time_pit_handler_1(time):
    ...
    global integral_w_y_l, integral_w_y_r, in_w_y_last_l,in_w_y_last_r, w_y_out_l, w_y_out_r , v_out_l_last, v_out_r_last 
    ...
    if run_flag:
        ...
        if ticker_count % 2 == 0:
            pur_theita_l = pur_theita + v_out_l_last + (v_out_l - v_out_l_last) * round(ticker_count / 2 - 0.5) / 5
            pur_theita_r = pur_theita + v_out_r_last + (v_out_r - v_out_r_last) * round(ticker_count / 2 - 0.5) / 5
            #pur_theita_l = pur_theita
            #pur_theita_r = pur_theita
...
while True:
    ...
    if cal_flag :
        ...
        w_z_last = w_z_now
        wireless.send_oscilloscope(left_m, right_m, left_f, right_f, v_turn, v_l, v_r)
