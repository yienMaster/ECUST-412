...
def time_pit_handler_1(time):
    ...
    if run_flag:
        if turn_count == 3:
            ...
            cal_flag = 1
            turn_count = 0
            if abs(v_turn_last - v_turn) <= 200:
                v_turn_last = v_turn
            elif abs(v_turn_last - v_turn) <= 400:
                v_turn = 0.2 * v_turn_last + 0.8 * v_turn
                v_turn_last = v_turn
            elif abs(v_turn_last - v_turn) <= 800:
                v_turn = 0.3 * v_turn_last + 0.7 * v_turn
                v_turn_last = v_turn
            else:
                if v_turn_last - v_turn > 0:
                    v_turn = v_turn_last - 800
                    v_turn_last = v_turn
                else:
                    v_turn = v_turn_last + 800
                    v_turn_last = v_turn
...
while True:
    ...
    if cal_flag :
        ...
        wireless.send_oscilloscope(left_m, right_m,left_f, right_f,line_pos,v_turn)
            ...
            if mode_2 == 1:
                ...
                #左侧环岛
                    ...
                    if mode_4 == 0:
                        ...
                        if right_f < 53 and last_right_f < 53:
                            sign_sci_f = 1
                            sign_sci_m = 1
                            sci_num = 0
                            mode_4 = 5
                    ...
                    if mode_4 != 3 and mode_4 != 4 and mode_4 != 5:
                        ...
                        #if left_m < - 40:
                        left_m = right_m - road_width_f*0.8
                        #if left_f < -30:
                        left_f = right_f - road_width_m*0.8
