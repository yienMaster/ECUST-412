...
def time_pit_handler_1(time):
    ...
    if run_flag:
        ...
        if abs(v_turn) <= 900:
            motor_duty_l = w_y_out_l + 1 * v_turn
            motor_duty_r = w_y_out_r - 1 * v_turn
        elif 900 < abs(v_turn) <= 1500:
            motor_duty_l = w_y_out_l + 0.96 * v_turn
            motor_duty_r = w_y_out_r - 0.96 * v_turn
        elif 1500 < abs(v_turn) <= 2500:
            motor_duty_l = w_y_out_l + 0.94 * v_turn
            motor_duty_r = w_y_out_r - 0.94 * v_turn
        else:
            motor_duty_l = w_y_out_l + 0.92 * v_turn
            motor_duty_r = w_y_out_r - 0.92 * v_turn
        '''
        motor_duty_l = w_y_out_l + 3000 * math.atan(v_turn)
        motor_duty_r = w_y_out_r - 3000 * math.atan(v_turn)
        '''
        
        #(motor_duty_l, motor_duty_r,v_l, v_r, pur_theita_l, v_out_r , pur_v)
        ...
        if mode_1 == 1:
            ...
            if mode_2 == 1:
                ...
                if mode_3 == 1:
                    ...
                    #wireless.send_oscilloscope(left_m, right_m,left_f, right_f, v_turn)
