...
while True:
    ...
    # 去掉前11个数和后8个数
    modified_list1 = processed_data1[12:-12]
    modified_list2 = ccd_data1[4:-1]
    
    convolved_data, eroded_data, dilated_data = process_ccd_data(ccd_data2)
    processed_data2 = threshold_and_denoise(dilated_data)
    
    # 去掉前11个数和后8个数
    modified_list3 = processed_data1[9:-9]
    modified_list4 = ccd_data1[4:-1]
    ...
    lcd.wave(0,  0, 128, 64, modified_list1, max = 255)
    lcd.wave(0, 80, 128, 64, modified_list2, max = 255)
    lcd.wave(0, 160, 128, 64, modified_list3, max = 255)
    lcd.wave(0, 240, 128, 64, modified_list4, max = 255)
    
    # 计算100次的平均位置（假设processed_data_list有100次数据）
    for i in range(128):  # 从0到127
        if processed_data2[i] == 100:
            del_pos_l.pop(0)
            del_pos_l.append(i)
            break
        
    for i in range(128):  # 从0到127
        if processed_data2[127 - i] == 100:
            del_pos_r.pop(0)
            del_pos_r.append(i)
            break
