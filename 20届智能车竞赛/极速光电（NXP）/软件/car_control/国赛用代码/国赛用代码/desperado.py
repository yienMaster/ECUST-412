...
def follow_line(pur, data, gyro , kp, kd, kd_w_z, d_corrcte_last, v_l, v_r, v_pur , in_last):
    ...
    in_now = pur - data
    offset = in_now * in_now * in_now / 7200 + in_now * 0.075
...
def time_pit_handler_1(time):
    ...
    if run_flag:
        ...
        if ticker_count % 10 == 0:
            ...
            if pur_v != 0:
                if abs(v_l) >= pur_v + 60 or abs(w_z_real) > 35:
                    stop_flag = 1
        ...
        if abs(encoder_l.get()) >= 160 or abs(encoder_r.get()) >= 160:
            dis_jump = 0.8
        else:
            dis_jump = 1
