...
def follow_line(pur, data, gyro , kp, kd, kd_w_z, d_corrcte_last, v_l, v_r, v_pur , in_last):
...
#     out = ((v_real * v_real + 1)/ (v_pur * v_pur + 1) + 1 )* kp * offset - d_corrcte * kd * gyro
    out = math.log(1 + 0.3 * v_real ** 2 + 0.2 * v_pur ** 2) * kp * offset +  kd_w_z * gyro +d_corrcte * kd * (in_now - in_last)
...
def time_pit_handler_1(time):
    ...
    global integral_w_y_l, integral_w_y_r, in_w_y_last_l,in_w_y_last_r, w_y_out_l, w_y_out_r, dis_l_r
    global pur_theita_l, pur_theita_r, in_theita_last_l, in_theita_last_r, pur_w_y_l, pur_w_y_r, pur_theita
    global v_out_l_last, v_out_r_last
    ...
    if(imu_data[1] == 0):
        ...
        a = imu_data[1]
    theita_1 = math.atan2(imu_data[2]-50 , a)

    w_y_dps = imu_data[3] - 2
    ...
    if run_flag:
        if turn_count == 3:
            ...
            #print(1)
            d_corrcte_last,in_dis_last, v_turn = follow_line(line_pos, 0, w_z_real , kp_turn, kd_turn, kd_turn_w,  d_corrcte_last, v_l, v_r, pur_v, in_dis_last)
            cal_flag = 1
            turn_count = 0
        ...
        if ticker_count % 10 == 0:
            ...
            if pur_v != 0:
                if abs(v_l) >= pur_v + 20 or abs(w_z_real) > 35:
                    stop_flag = 1
                
                if abs(line_pos) >= 8:
                    real_pur_v = pur_v_turn
                else:
                    real_pur_v = pur_v
                ...
                if v_l >= pur_v_slow:
                    real_pur_v = pur_v_slow
            else:
                real_pur_v = 0
            
            #wireless.send_oscilloscope(real_pur_v)
            v_out_l_last = v_out_l
            v_out_r_last = v_out_r
            ...
            if sign_i == 0:
                ...
                if v_l <= 60:
                    ...
                    integral_v_l, in_v_last_l, v_out_l = v_pid(real_pur_v, v_l, 4 * kp_v_l, 30 * ki_v_l, kd_v_l, integral_v_l, in_v_last_l, inte_v_limit)
                    integral_v_r, in_v_last_r, v_out_r = v_pid(real_pur_v, v_r, 4 * kp_v_r, 30 * ki_v_r, kd_v_r, integral_v_r, in_v_last_r, inte_v_limit)
        
        if ticker_count % 2 == 0:
            pur_theita_l = pur_theita + v_out_l * round(ticker_count / 2 - 0.5) / 5
            pur_theita_r = pur_theita + v_out_r * round(ticker_count / 2 - 0.5) / 5
            
            #wireless.send_oscilloscope(pur_theita_l, pur_theita_r)
            ...
            #wireless.send_oscilloscope(abs(pur_theita_l-theita_cal))
        ...
        #wireless.send_oscilloscope(encoder_l.get(), encoder_r.get(), line_pos)
        ...
        #wireless.send_oscilloscope(w_y_out_l)
        
        #motor_duty_l = w_y_out_l + v_turn
        #motor_duty_r = w_y_out_r - v_turn
        
        if abs(encoder_l.get()) >= 170 or abs(encoder_r.get()) >= 170:
            dis_jump = 0.8
        else:
            dis_jump = 1
        
        if v_turn < 0:
            motor_duty_l = (w_y_out_l + v_turn * (1 + dis_l_r)) * dis_jump
            motor_duty_r = (w_y_out_r - v_turn * (1 - dis_l_r)) * dis_jump
        else:
            motor_duty_l = (w_y_out_l + v_turn * (1 - dis_l_r)) * dis_jump
            motor_duty_r = (w_y_out_r - v_turn * (1 + dis_l_r)) * dis_jump
...
def time_pit_handler_2(time):
    global key_flag, control_choose, screen_change
    global key_count, run_flag, sta_left_sci,sta_right_sci,sta_bar,sta_ramp,sta_stop,sta_screen
    global kp_turn , kd_turn, kd_turn_w, CCD_thr, pur_theita,pur_v ,pur_v_turn ,pur_v_slow, list_pos
    global m, f, m_l, m_r, f_l, f_r, dis_l_r
    ...
    if sta_screen:
        if key_count == 100:
            ...
            if key_data[0]:
                ...
                list_pos = (list_pos + 1) if (list_pos < 17) else (1)
                key.clear(2)
            if key_data[2]:
                control_choose = 1
                key.clear(3)
            ...
            if control_choose != 0:
                ...
                if list_pos == 1:
                    sta_left_sci = 0 if (sta_left_sci == 1) else (1)
                elif list_pos == 2:
                    sta_right_sci = 0 if (sta_right_sci == 1) else (1)
                elif list_pos == 3:
                    sta_bar = 0 if (sta_bar == 1) else (1)
                elif list_pos == 4:
                    sta_ramp = 0 if (sta_ramp == 1) else (1)
                elif list_pos == 5:
                    sta_stop = 0 if (sta_stop == 1) else (1)
                elif list_pos == 6:
                    kp_turn = kp_turn + control_choose * 5
                elif list_pos == 7:
                    kd_turn = kd_turn + control_choose * 5
                elif list_pos == 8:
                    kd_turn_w = kd_turn_w + control_choose * 100
                elif list_pos == 9:    
                    CCD_thr = CCD_thr + control_choose * 5
                    
                elif list_pos == 10:
                    pur_v = pur_v + control_choose * 5
                elif list_pos == 11:
                    pur_v_turn = pur_v_turn + control_choose * 5
                elif list_pos == 12:
                    pur_v_slow = pur_v_slow + control_choose * 5
                    
                elif list_pos == 13:
                    m = m + control_choose * 0.1
                    f = 1 - m
                elif list_pos == 14:
                    m_r = m_r + control_choose * 0.1
                    f_r = 1 - m_r
                elif list_pos == 15:
                    m_l = m_l + control_choose * 0.1
                    f_l = 1 - m_l
                elif list_pos == 16:       
                    dis_l_r = dis_l_r + control_choose * 0.1
                elif list_pos == 17:       
                    pur_theita = pur_theita + control_choose * 0.001
...
while True:
    ...
    #wireless.send_oscilloscope(,theita_cal)
    #读取传感器   
    if cal_flag :
        ...
        left_m, right_m, road_width_m_now = find_road(ccd_data2 , road_width_m, CCD_thr)
        left_f, right_f, road_width_f_now = find_road(ccd_data3 , road_width_f, CCD_thr)
        #wireless.send_oscilloscope(real_pur_v)
        #wireless.send_oscilloscope(encoder_l.get(), encoder_r.get(), line_pos, mode_1)
        #wireless.send_oscilloscope(road_width_m_now, road_width_f_now, mode_4, mode_1,line_pos)
        ...
        #wireless.send_oscilloscope(left_m, right_m, left_f, right_f, w_z_real)
        ...
        if mode_1 == 1:
            ...
            if mode_2 == 1:
                ...
                if stop_num != 0:
                    line_pos = 0
                #wireless.send_oscilloscope(road_width_m, road_width_f)
            ...
            if mode_2 == 1:
                ...
                if mode_3 == 0:
                    ...
                    if sta_left_sci == 1 and (((left_f - last_left_f)<= -15) and left_f <= -50 and abs(right_f-road_width_f/2)<10):
                        sign_sci_l = 1

                    elif sta_right_sci == 1 and (((right_f - last_right_f)>= 15) and right_f >= 50 and abs(left_f+road_width_f/2)<10):
                        sign_sci_r = 1
                    ...
                    if mode_4 == 0:
                        ...
                        if sci_mode_5_num < 30:
                            sci_mode_5_num = sci_mode_5_num + 1
                            
                        if  (abs(road_width_m_now - 70) <= 15 and abs(left_m+right_m) <=20 and left_m > -50) and (abs(road_width_f_now - 50) <= 10 and abs(left_f+right_f) <= 20 and left_f > -40):
                    ...
                    if mode_4 != 3 and mode_4 != 4 and mode_4 != 5 and mode_4 != 2:
                        ...
                        if sign_sci_m == 0:
                            if left_m == -53:
                                right_m = left_m + road_width_m*1.15
                            else:
                                right_m = left_m + road_width_m*1.05
                        else:
                            if right_m == 53:
                                left_m = right_m - road_width_m*0.65
                            else:
                                left_m = right_m - road_width_m*0.75
                        
                        if sign_sci_f == 0:
                            if left_f == -53:
                                right_f = left_f + road_width_f*1.15
                            else:
                                right_f = left_f + road_width_f*1.05
                        else:
                            if right_f == 53:
                                left_f = right_f - road_width_f*0.65
                            else:
                                left_f = right_f - road_width_f*0.75
                    ...
                    if mode_4 != 4:
                        ...
                        line_pos = - 18.5* (1/30) * (30 - sci_mode_5_num) + sci_mode_5_num * (m_l * (left_m + right_m) + f_l * (left_f + right_f))*(1/2) * (1/30)
                    ...
                    if mode_4 == 0:
                        ...
                        if (abs(road_width_m_now - 70) <= 15 and abs(left_m+right_m) <=20 and right_m < 50) and (abs(road_width_f_now - 50) <= 10 and abs(left_f+right_f) <=20 and right_f < 40):
        ...
        if sta_screen == 1:
            if screen_num == 1:
                ...
                lcd.str12(10,  0,  "LEFT_SCI", 0xFFFF)
                lcd.str12(10, 18,  "RIGHT_SCI", 0xFFFF)
                lcd.str12(10, 36,  "BAR", 0xFFFF)
                lcd.str12(10, 54,  "RAMP", 0xFFFF)
                lcd.str12(10, 72,  "STOP", 0xFFFF)
                lcd.str12(10, 90,  "kp_turn", 0xFFFF)
                lcd.str12(10, 108, "kd_turn", 0xFFFF)
                lcd.str12(10, 126, "kd_turn_w", 0xFFFF)
                lcd.str12(10, 144, "CCD_thr", 0xFFFF)
                lcd.str12(10, 162, "pur_v", 0xFFFF)
                lcd.str12(10, 180, "pur_v_turn", 0xFFFF)
                lcd.str12(10, 198, "pur_v_slow", 0xFFFF)
                lcd.str12(10, 216, "m", 0xFFFF)
                lcd.str12(10, 234, "m_r", 0xFFFF)
                lcd.str12(10, 252, "m_l", 0xFFFF)
                lcd.str12(10, 270, "dis_l_r", 0xFFFF)
                lcd.str12(10, 288, "pur_theita", 0xFFFF)
            ...
            for i in range(1, 17 + 1):
                ...
                lcd.str12(0, height, mark, 0xFFFF)
            
            if screen_change == 1:   
                
                if sta_left_sci == 1:
                    lcd.str12(130, 0,  a, 0xFFFF)
                else:
                    lcd.str12(130, 0,  b, 0xFFFF)
                    
                if sta_right_sci == 1:
                    lcd.str12(130, 18,  a, 0xFFFF)
                else:
                    lcd.str12(130, 18,  b, 0xFFFF)
                    
                if sta_bar == 1:
                    lcd.str12(130, 36,  a, 0xFFFF)
                else:
                    lcd.str12(130, 36,  b, 0xFFFF)
                    
                if sta_ramp == 1:
                    lcd.str12(130, 54,  a, 0xFFFF)
                else:
                    lcd.str12(130, 54,  b, 0xFFFF)
                    
                if sta_stop == 1:
                    lcd.str12(130, 72,  a, 0xFFFF)
                else:
                    lcd.str12(130, 72,  b, 0xFFFF)
                
                lcd.str12(110, 90,  "%5d"%kp_turn, 0xFFFF)
                lcd.str12(110, 108,  "%5d"%kd_turn, 0xFFFF)
                lcd.str12(110, 126,  "%5d"%kd_turn_w, 0xFFFF)
                
                lcd.str12(110, 144,  "{:>3d}".format(CCD_thr), 0xFFFF)
                
                lcd.str12(110, 162, "%3d"% pur_v, 0xFFFF)
                lcd.str12(110, 180, "%3d"% pur_v_turn, 0xFFFF)
                lcd.str12(110, 198, "%3d"% pur_v_slow, 0xFFFF)
                
                lcd.str12(110,216,  "%5.1f"%m, 0xFFFF)
                lcd.str12(110,234,  "%5.1f"%m_r, 0xFFFF)
                lcd.str12(110,252,  "%5.1f"%m_l, 0xFFFF)
                lcd.str12(110,270,  "%5.2f"%dis_l_r, 0xFFFF)
                    
                lcd.str12(110, 288,  "{:>5.3f}".format(pur_theita), 0xFFFF)
