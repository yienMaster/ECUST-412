...
def ccd_road_find(ccd_data):
    ...
    return pos - mid_point  # 如果没有找到交界点，返回None和0
...
def follow_road(line_pos, w_z_real, v_l, v_r, theita_real, w_y_real):
    ...
    global in_theita_last_l, in_theita_last_r, integral_w_y_l, integral_w_y_r, pur_v
    '''转向环'''
    ...
    if(motor_duty_l >= 9000):
        motor_duty_l = 9000
    elif(motor_duty_l <= -9000):
        motor_duty_l = -9000
    
    if(motor_duty_r >= 9000):
        motor_duty_r = 9000
    elif(motor_duty_r <= -9000):
        motor_duty_r = -9000
    
    #更新电机PWM输出
    #wireless.send_oscilloscope(pid_out_l,pid_out_r, pur_theita_l)
...
def time_pit_handler_1(time):
    ...
    follow_road(0, 0, v_l, v_r, theita_cal, w_y_cal)
...
while True:
    #读取传感器
    ccd_data1 = ccd.get(0)
    #wireless.send_oscilloscope(v_l,)
    ticker_flag = False
    imu_data = imu.get()
    ...
    '''        
    lcd.str12(0, 0, "{:.3f},{:.3f},{:.3f},{:.3f}.".format(data_wave[0], data_wave[1], data_wave[2], data_wave[3]), 0xF800)
    '''
    ...
    #wireless.send_oscilloscope(pur_theita)
    pur_v = data_wave[5]
    
    #line_pos = data_wave[2]
    '''
    ...
    kp_v_r = kp_v_l
    ki_v_l = data_wave[3] * 0.0000001
    ki_v_r = ki_v_l
    
    '''
    lcd.str24(0,0,"Kp_theita={:.3f}.".format(kp_theita_l),0x001F)
    lcd.str24(0,28,"kd_theita={:.3f}.".format(kd_theita_l),0x001F)
    lcd.str24(0,56,"Kp_w_y={:.3f}.".format(kp_w_y_l),0x001F)
    lcd.str24(0,84,"Ki_w_y={:.3f}.".format(ki_w_y_l),0x001F)
    '''
    '''
    位姿解算和卡尔曼滤波
    '''
    ...
    道路检测
    '''
    line_pos = ccd_road_find(processed_data1)
