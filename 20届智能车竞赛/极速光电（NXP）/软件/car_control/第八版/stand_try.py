...
def follow_road(line_pos, w_z_real, v_l, v_r, theita_real, w_y_real):
    ...
    #更新电机PWM输出
    #wireless.send_oscilloscope(motor_duty_l, kp_theita_l, kd_theita_l,kp_w_y_l, ki_w_y_l)
...
def time_pit_handler_1(time):
    global v_l,v_r,imu_data
    v_l = encoder_l.get()
    v_r = encoder_r.get()
    #wireless.send_oscilloscope(w_y_cal,w_y)
    follow_road(car_pos, 0, 0, 0, theita_cal, w_y_cal)

pit = ticker(1)
# 关联采集接口
pit.capture_list(encoder_l, encoder_r,imu)
# 关联 Python 回调函数
